...
::: {.callout-important}
### Details for the final project are a work-in-progress

The details for the final project are still being finalized for the spring 2026 semester. Please consider this preliminary guidance and look out for additional instructions.
:::
...
<!-- See the readings and materials from [week 9](/weeks/week_09.qmd) or [week 10](/weeks/week_10.qmd) for more background information on these sources. -->
...
**Always ask yourself:** Who might benefit from your proposed project? How can your project can avoid causing people harm?
...
-   **project data (if needed)**: including the source files or, if files exceed the 50MB maximum size allowed on GitHub, a script used for importing and processing the data before visualization or analysis. Students who are using `{osmdata}` or `{tidycensus}` should include scripts for downloading data but are *not* required to include the source data.
