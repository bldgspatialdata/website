...
These slides are adapted from [Ch. 3 Data transformation](https://r4ds.hadley.nz/data-transform.html) in R for Data Science (2e).
...
select(
  storms,
  any_of(c("lat", "latitude", "y", "lon", "long", "longitude", "x"))
)
```
...
Your turn: use select to subset the variables for wind, pressure, tropical storm force diameter, and hurricane force diameter.
...
But it may be easier to use write the condition using the `%in%` operator:
...
You can use a negated logical vector to filter out values:
...
Or you can use the new `dplyr::filter_out()` function:
...
Your turn: Hurricane Lee has **hurricane-force winds** extending up to 115 miles from its center and **tropical storm-force winds** extending for some 240 miles.

Can you use filter to find the name and year of a hurricane with observed wind speeds that are the same or greater?
