...
- Phone camera photos (stored in the EXIF metadata accessible [via exiftoolr](https://joshobrien.github.io/exiftoolr/index.html))
...
[Open Geospatial Consortium (OGC) Standards](https://www.ogc.org/standards-overview/) are organized into categories:

- APIs (e.g. [Moving Features API](https://www.ogc.org/standards/ogc-api-moving-features/))
- Web Services (e.g. [Web Feature Service](https://www.ogc.org/standards/wfs/))
...
## Why are shapefiles bad?

- Difficult to manage multiple files

- Single geometry type per file

- Attribute names can't exceed 10 characters

[And more...](http://switchfromshapefile.org/#shapefileisbad)
...
- Local file databases
  ...
  - [duckdb](https://duckdb.org/) (see [duckspatial R package](https://cidree.github.io/duckspatial/))
...
```{r}
read_sf(
  "https://github.com/elipousson/marylandedu/raw/main/data-raw/extdata/Enrollment_By_Grade_2023.csv"
)
```
...
## Practice: Use a spatial query in `wkt_filter`

```{r}
wkt_filter <- st_as_text(nc[1, ]$geometry)
...
## Example: ArcGIS Feature Services with `read_sf`

An ArcGIS FeatureServer URL does *not* work:

```{r}
read_sf(
  "https://services.arcgis.com/njFNhDsUCentVYJW/arcgis/rest/services/MDOT_SHA_County_Boundaries/FeatureServer/0"
)
```
...
```{r}
read_sf(
  "https://services.arcgis.com/njFNhDsUCentVYJW/arcgis/rest/services/MDOT_SHA_County_Boundaries/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson"
)
```

## Example: ArcGIS Feature Services with `arc_select`

For ArcGIS Feature Services, the `{arcgislayers}` package is the recommended way to access data:

```{r}
library(arcgislayers)

feature_url <- arc_open(
  "https://services.arcgis.com/njFNhDsUCentVYJW/arcgis/rest/services/MDOT_SHA_County_Boundaries/FeatureServer/0"
)

arc_select(feature_url)
```

## Example: ArcGIS Portal Items with `arc_open`

`{arcgislayers}` also has support for getting data on "portal items" like web maps and applications:

```{r}
app <- arc_open(
  "https://maryland.maps.arcgis.com/apps/webappviewer/index.html?id=e4f351766e614b7195525b49b8be112e"
)

app
```

## Practice: Write a SQL expression to query a FeatureLayer

Use the format "{Field_name} {Operator} {Value or String}" to build a query and filter data before loading it to your computer.

Check out the [SQL reference for ArcGIS query expressions](https://pro.arcgis.com/en/pro-app/latest/help/mapping/navigation/sql-reference-for-elements-used-in-query-expressions.htm) for more information.

```{r}
url <- "https://services1.arcgis.com/UWYHeuuJISiGmgXx/arcgis/rest/services/311_Customer_Service_Requests_2024/FeatureServer/0"

layer <- arc_open(url)

arc_select(
  layer,
  where = "CloseDate > date '2024-01-01' AND CloseDate < date '2024-01-06'",
  n_max = 10
)
```
...
`mapgl::mapboxgl()` is now a recommended alternative to `addMapboxTiles()`.
